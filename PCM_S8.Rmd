...
Una manera muy práctica de represetar el proceso de diversificación es mediante lineaje through time o LTT.
Básicamentee lo que hacemos es contar el númeor de linajes que se van acumulando desde la raíz de la filogenia hasta el presente.
...
La teoría nos dice que en un escenario de speciación constante (sin extinción; pur-birth), los linajes se acumulan exponecialmente a lo largo del tiempo desde la raíz del árbol.
...
Para estudiar el proceso de diversificación en más detalle vamos a utilizar la filogenia de un grupo de peces (Near et lal., 2011). En este estudio se muestraron 201 de un total de 216 especie que constituyen el clado de peces (93%). Para cálculos de tasas de diversificación es necesario tener en cuenta ese muestreo incompleto que se ha hecho del grupo.
...
Y ya estamos listo para computar LTT.
...
ESTOMACIÓN DE TASAS DE ESPECIACIÓN Y EXTINCIÓN A PARTIR DE UNA FILOGENIA

Usaremos para calcular la tasas de especiación y extinción el método de Nee et al. (1992).
...
La pendiente de la recta de acumulación de linajes en escala semilogatirmica es en este caso el valor esperado de especiación o diversificación (ya que la extinción es 0; Nee et al., 1992, 1994a).
...
Un incremento de la pendiente hacia el presente es firma de la extinción. Y cuanto mayor sea el ratio extinción/especiación, mayor será la magnitud de esta curva.
Veamoslo mediante una simulación.
Con la función pbtree vamos a simular un árbol sin extinción (en este caso tenemos un recta en escala semilogaritmica) y está pendiente es la tasas de especiación.
También vamos a simular un arbol de tipo birth-death, es decir, con especiación y extinción, además, está extinción será muy elevada. Vamos a calcular el LTT de este árbol incluyendo los linakes extintos.
...
Esto lo vamos autilizar para calcular la tasa de especiación y extinción.

Usaremos la función fit.bd de phytools (Nee t al., 1994a).
...
Si volvemos a nuestro data set simulado podemos sacar alguna conclusiones.
...
Como hemos dicho antes el patrón de pull of the present aporta información sobre la extinción. Si no tenemos un muestreo completo de nuestro dataset se oscuerece este patron y la posibilidad de inferir información sobre el proceso de extinción.
Por lo tanto es esencial tener en cuenta las especies que no muestreadas en la filogenia cuando se infiere el patrón de diverisificación a partir de una filogenia.
...
Nuestro árbol se ajusta un proceso pure-birth.
...
Y vamos a de nuevo a ajustar los modelos.
...
La convegencia de este modelo se alcanza bastante rápido. Quitaremos como burn-inel %5 inicial de la iteraciones.
...
Como podemos ver los resultados están en concordancia con los estimados anteriormente. No podeoms descartar que la tasa de extinción sea cero.

####################

ACTIVIDADES

Usa el archivo 10kTrees_Primates.nex que hay en este proyecto de R. Este archivo contiene 100 árboles de primates muestreados a partir de la distribución posterior de un análisis bayesiano y se obtuvo del "servidor web 10kTrees". 28 Estos árboles están (en teoría) calibrados en el tiempo en millones de años. Utilice phytools o diversitree para estimar las tasas de especiación y extinción de todos los árboles de la muestra. Calcule las tasas medias de especiación y extinción, así como la variabilidad en la tasa entre árboles. Tenga en cuenta la fracción de muestreo incompleta. Existe cierto debate sobre cuántas especies de primates existen actualmente. Varias fuentes afirman que el orden de los mamíferos Primates puede incluir de 376 a 522 especies. Establezca la fracción de muestreo para asumir estos valores diferentes para la diversidad total de especies del orden. ¿Qué efecto tienen estas diferentes diversidades supuestas en nuestra especiación estimada y ex?
